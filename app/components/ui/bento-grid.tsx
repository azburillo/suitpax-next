...
import { TrendingUpIcon, CheckCircleIcon, GlobeAltIcon } from "@heroicons/react/24/outline"
...
function BentoGrid({ items = defaultItems }: BentoGridProps) {
  // items prop now defaults to defaultItems
  return (
            ...
            <div className="flex items-center justify-between mt-2">
              <div className="flex items-center space-x-2 text-xs text-gray-500 dark:text-gray-400">
                {item.tags?.map((tag, i) => (
                  <span
                    key={i}
                    className="px-2 py-1 rounded-md bg-/5 dark:bg-white/10 backdrop-blur-sm transition-all duration-200 hover:bg-black/10 dark:hover:bg-white/20"
                  >
                    #{tag}
                  </span>
                ))}
